...
export default function Contact(){
  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      <h1 className="text-3xl font-bold text-indigo-900 mb-4">Contact Us</h1>
      ...
      <form onSubmit={submit} className="space-y-4 bg-white p-6 rounded shadow">
        <input className="w-full p-3 border border-slate-300 rounded" placeholder="Your name (optional)" value={name} onChange={e=>setName(e.target.value)} />
        <input className="w-full p-3 border border-slate-300 rounded" placeholder="Your email" value={email} onChange={e=>setEmail(e.target.value)} required />
        <textarea className="w-full p-3 border border-slate-300 rounded min-h-[140px]" placeholder="How can we help?" value={message} onChange={e=>setMessage(e.target.value)} required />
        <div className="flex justify-between items-center">
          ...
          <button type="submit" disabled={sending} className="px-4 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700">{sending ? 'Sending...' : 'Send Message'}</button>
        </div>
      </form>
    </div>
  );
